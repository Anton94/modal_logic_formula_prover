...
{
	if (root.op_ == operation_t::intersaction_ || root.op_ == operation_t::union_ || root.op_ == operation_t::star_)
	{
	...
	if (root.op_ == operation_t::star_)
	{
		...
		else if (root.childs_.left->op_ == operation_t::union_ || root.childs_.left->op_ == operation_t::intersaction_)
		{
			if (root.childs_.left == nullptr)
			{
				throw "fail";
			}
