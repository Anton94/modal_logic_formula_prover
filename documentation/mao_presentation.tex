...
\begin{itemize}
	...
	\item Табло метод в съждителната логика
\end{itemize}
\end{frame}

\subsection{Табло метод в съждителната логика}
\begin{frame}\frametitle{Табло метод в съждителната логика}
...
\newline
\newline
Използваме табло метода за да опростим формулата и търсим модел в клоновете на табло метода.
\newline
\newline
		Нека $\varphi$ е формула и $\mathcal{T}$ е таблото от $\varphi$, то клона на таблото има следните маркирани атомарни формули:
		\begin{align*}
...
\begin{defn}[Оценка на променливи]
	С $\curvedE$ ще ознчаваме функцията, която за всяка променлива от $\Var_B$ дава истина или лажа.
		\begin{align*}
...
\begin{frame}\frametitle{Построяване на модална точка за не-равен на нула терм}
	За всеки не-равен на нула терм $a \neq 0 \in B$ ще построим една модални точки, такава, че $\vBool_{\curvedE}(a) = \textbf{истина}$. \\
	За равно на нула термовете, следните условия трябва да са изпълнени:
	\begin{align*}
		t = 0 \in B: \vBool_{\curvedE}(t) = \textit{лъжа}
	\end{align*}
	\begin{align*}
		\neg C(e, f) \in B: (\vBool_p(e) = \textbf{лъжа} \textit{ and } \vBool_p(f) = \textbf{лъжа})
	\end{align*}
...
\begin{frame}\frametitle{Построяване на модел}
	\begin{lema}
		...
		\begin{align*}
			\vBool_{\curvedE}(a) = \textit{ истина } \leftrightarrow \curvedE \in \vE(a)
		\end{align*}
