...
\begin{frame}{Свързана контактна логика}
	...
	\uncover<2->{
		\begin{equation*}
			\upsilon(a) \neq \emptyset \land \upsilon(a) \neq W \implies (\exists x \in \upsilon(a)) (\exists y \in W \setminus \upsilon(a))(xRy)
		\end{equation*}
	}
\end{frame}
...
\begin{frame}
	\uncover<1->{
		Релационната система $\F = (W, R)$ дефинира ненасочен граф G(W, R). $W$ е множеството от върхове, а $R$ е множеството от ребра.
	}
	...
	\uncover<3->{
		\begin{definition}[Свързан граф]
			Нека $G = (W, R)$ е ненасочен граф. $G$ е \textbf{свързан}, ако има път между всеки два различни върха в $W$.
			\begin{equation*}
	...
	\uncover<1->{
		\begin{theorem}[Свързаност]
		 Нека $\F = (W, R)$ е релационна система и G = (W, R) е графът, дефиниран от нея.
			\begin{center}
				аксиомата за свързаност е удоволетворена в $\F \iff$ G е свързан
			\end{center}
		\end{theorem}
	}
...
\begin{frame}{Свързан модел}
	...
	\uncover<2->{
		\begin{step}
			$\F^v = (W^v, R^v)$, $\M^v = (\F^v, \upsilon)$. $\M^v$ е модел на $\beta$, ако контактите и ненулевите термове в $\beta$ са удоволетворени. Ако $\M^v$ не е модел, тогава $\beta$ няма модел(нито свързан модел).
		\end{step}
	}
\end{frame}
...
\begin{frame}{Запазване удоволетворимост атомарни формули}
	\begin{lemma}[Запазване удоволетворимост атомарни формули]
...
\begin{frame}{Свързан модел}
	\begin{step}
		Нека $\M^v$ е модел на $\beta$. Всички модели, дефинирани от свързаните компоненти на $G^v$ запазват удоволетворимостта на нулевите термове и не-контактите(не добавят точки, нито релации). Ако има свързана компонента, която запазва удоволетворимостта на контактите и ненулевите термове, то тя дефинира \textbf{свързан модел} на $\beta$.
...
\begin{frame}{Максимално свързани компоненти}
	\begin{definition}[$Comp^G$]
		Нека $G=(W, R)$ е граф.
		\begin{center}
			$Comp^G = \{ G' \mid G' \subseteq G$ и G' е максимлано свързана компонента $\}$
		\end{center}
	\end{definition}
\end{frame}
...
\begin{frame}{Свързан модел}
	\begin{step}
		Нека $Comp^{G^v}$ е множеството от максимлано свързани компоненти на $G^v$. Нека $|W^v|=m$, тогава $|Comp^{G^v}| <= m$.
	\end{step}

	\begin{step}
		Всеки модел дефиниран от граф в $Comp^{G^v}$ удоволетворява нулевите термове и не-контактите на $\beta$.  Ако има такъв, който запазва удоволетворимостта на контактите и ненулевите термове, то той е \textbf{свързан модел} на $\beta$. Ако няма, то $\beta$ няма свързан модел.
	\end{step}
\end{frame}
