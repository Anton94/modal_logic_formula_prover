...
\subsection{Measure formula satisfiability}
	...
	A modal point is represented by a valuation function defined in \ref{variable-valuation}.
	To prove that a model exists each subset of valid modal points should be tested whether it satisfies the branch conjunction.
	...
	Let W be a set of valid points, then for each valid point $\curvedE$ we can determine if it is present in the model valuation of an arbitrary term. Namely we can define the model valuation $\vE$ as follows:
	\begin{align*}
		\vE^W(m) = \{ p \; | \; \vBool_{\curvedE}(m) \; \textit{is true} \}
	\end{align*}
	\end{defn}
	...
	\begin{defn}%{Valid Measure Model} 
	...
	$\mathcal{M}$ is a possible model where the equal to zero and non-contacts formulas are satisfied. For $\mathcal{M}$ to be a model for the branch conjunction B,
	...
	it does not mean that the branch conjunction B does not construct a model.

	It is possible that the satisfying model has less modal points. 
	This imposes that we should test the satisfiability of the branch conjunction against all models produced from the modal points for each subset of W.
	...
	\begin{lemma}[Valuation deduction] \label{valuation-deduction}
		...
		Let $\{ q_0, ... q_n \} \subseteq W$ be valid modal points and let W' be defined as $W' = W \textbackslash \{ q_0, ... q_n \}$, then for the terms in the non-equal to zero, contacts, measure less or equal and measure less formulas the $\vE^{W'}$ can be calculated as follows:
		\begin{itemize}
			\item $t \neq 0: \vE^{W'}(t) =  \vE^W(t) \textbackslash \{ q_0, ... q_n \}$
			\item $C(a,b): \vE^{W'}(a) =  \vE^W(a) \textbackslash \{ q_0, ... q_n \} \textit{ and } \vE^{W'}(b) =  \vE^W(b) \textbackslash \{ q_0, ... q_n \}$
			\item $a \leq_\mu b: \vE^{W'}(a) =  \vE^W(a) \textbackslash \{ q_0, ... q_n \} \textit{ and } \vE^{W'}(b) =  \vE^W(b) \textbackslash \{ q_0, ... q_n \}$
			\item $a <_\mu b: \vE^{W'}(a) =  \vE^W(a) \textbackslash \{ q_0, ... q_n \} \textit{ and } \vE^{W'}(b) =  \vE^W(b) \textbackslash \{ q_0, ... q_n \}$
		\end{itemize}
	\end{lemma}
...
\newline
	...
	\begin{enumerate}
		\item Generate all valid modal points W over B.% This implies the model $\mathcal{M} = (W, R, \vE)$.
