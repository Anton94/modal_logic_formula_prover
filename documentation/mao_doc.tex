...
\subsection{Measure formula satisfiability}
	...
	\newline
		...
		Let W' be defined as $W \textbackslash \{ p \}$ for an arbitrary modal point p. This way we will see that if we remove an arbitrary modal point from
		...
		\begin{itemize}
			...
			\item $t \equiv \neg u$, where u is a term and $\vE(\neg u) = \emptyset$ \\
				from $\vE(\neg u) = W \textbackslash \vE(u) = \emptyset$ it follows that $\vE(u) = W$ \\
				$\Rightarrow \neg \exists q \in W: q \in \vE(\neg u )$ \\
				$\Rightarrow \forall q \in W: q \in \vE(u)$, from $W' = W \textbackslash \{p\}$, it follows that \\
				$ \neg \exists q \in W': q \in \vE'(\neg u )$ \\
			...
			\item $t \equiv u \sqcap w$, where u and w are terms and $\vE(u \sqcap w) = \emptyset$ \\
				...
				$\Rightarrow \forall q \in W: q \notin \vE(u) \textit { or } q \notin \vE(w)$, from $W' = W \textbackslash \{p\}$, it follows that \\
				$\Rightarrow \forall q \in W': q \notin \vE'(u) \textit { or } q \notin \vE'(w)$
	...
	\begin{lemma}[Valuation deduction] \label{valuation-deduction}
		...
		Let $\{ q_0, ... q_n \}$ be valid modal points and let W' be defined as $W' = W \textbackslash \{ q_0, ... q_n \}$, then $\vE'$ can be calculated as:
		\begin{itemize}
			\item $t \neq 0 \in B: \vE'(t) =  \vE(t) \textbackslash \{ q_0, ... q_n \}$
			\item $C(a,b) \in B: \vE'(a) =  \vE(a) \textbackslash \{ q_0, ... q_n \} \textit{ and } \vE'(b) =  \vE(b) \textbackslash \{ q_0, ... q_n \}$
			\item $a \leq_\mu b \in B: \vE'(a) =  \vE(a) \textbackslash \{ q_0, ... q_n \} \textit{ and } \vE'(b) =  \vE(b) \textbackslash \{ q_0, ... q_n \}$
			\item $a <_\mu b \in B: \vE'(a) =  \vE(a) \textbackslash \{ q_0, ... q_n \} \textit{ and } \vE'(b) =  \vE(b) \textbackslash \{ q_0, ... q_n \}$
		\end{itemize}
	\end{lemma}
	
	This lemma implies that the valuation for each subset of W can be deduced from $\vE$ with less computations.

	\paragraph{Measure formula satisfiability algorithm} The satisfiability of a measure formula is defined with the following steps:
	\begin{enumerate}
		\item Generate all valid modal points W, which implies the model $\mathcal{M} = (W, R, \vE)$.
		\item Compute ahead the valuations of the $t \neq 0, C(a,b), a \leq_\mu b, a <_\mu b$ formulas given in the valuation deduction lemma \ref{valuation-deduction}
		\item For each subset $W' \subseteq W$ test if it satisfies the $t \neq 0, C(a,b)$: 
		\begin{enumerate}
		\item In case it satisfies the $t \neq 0, C(a,b)$ verify whether the system of inequality has a solution. If the system has a solution then a valid model for the formula is constructed, otherwise continue with the next subset $W'$.
