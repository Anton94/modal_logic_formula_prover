...
For the purposes of our algorithm the elements of W will be such variable valuations. The variable valuations depends on the number of variables in the formula, this bounds the world W to have at most $2^n$ possible elements. From now on we will use the term modal point and variable valuation interchangeably.
	...
	\paragraph{Contact modal point construction step}\label{def-contacts-construction}
		Let B be a branch conjunction and $C(a, b) \in B$, where a and b are two terms. Then two modal points are generated $\curvedE^1$ and $\curvedE^2$ such that:
		\begin{itemize}
		...
		\begin{align*}
			 \neg C(e, f) \in B: (\vBool_{\curvedE^1}(e) = \textbf{false} & \textit{ or } \vBool_{\curvedE^2}(f) = \textbf{false}) \; \textit{ and } \\
				(\vBool_{\curvedE^1}(f) = \textbf{false} & \textit{ or } \vBool_{\curvedE^2}(e) = \textbf{false}) \; \textit{ and } \\
				(\vBool_{\curvedE^1}(e) = \textbf{false} & \textit{ or } \vBool_{\curvedE^1}(f) = \textbf{false}) \; \textit{ and } \\
				(\vBool_{\curvedE^2}(e) = \textbf{false} & \textit{ or } \vBool_{\curvedE^2}(f) = \textbf{false})
		\end{align*}

	To validate that the newly generated modal points satisfy the equal to zero terms, the following must be true:
	\begin{align*}
		t = 0 \in B: \vBool_{\curvedE^1}(t) = \textit{false} \text{ and }  \vBool_{\curvedE^2}(t) = \textit{false}
	\end{align*}
	...
	\begin{itemize}
		\item $\curvedE^1R\curvedE^1$ - reflexivity of the modal point $\curvedE^1$
		\item $\curvedE^2R\curvedE^2$ - reflexivity of the modal point $\curvedE^2$
		\item $\curvedE^1R\curvedE^2$ - symmetric relation between $\curvedE^1$ and $\curvedE^2$
		\item $\curvedE^2R\curvedE^1$ - symmetric relation between $\curvedE^2$ and $\curvedE^1$
	\end{itemize}
	...
	\paragraph{Measure formula properties} The following lemmas define useful properties of a measure model and measure formulas
...
\end{lemma}
	...
	\subsection{System of Inequalities} 
		...
		\begin{defn} \label{measure_less_equal_without_duplication}
		...
		\begin{defn} \label{measure_less_without_duplication}
		...
		\begin{defn}
			...
			\begin{itemize}
				\item inequalities for all $\le_\mu$ fromulas by def \ref{measure_less_equal_without_duplication}
				\item inequalities for all $<_\mu$ fromulas by def: \ref{measure_less_without_duplication}
...
\subsection{Measure formula satisfiability}
	...
	in the atomic formulas in that branch conjunction. The reason behind this is because of the global property of the measure atomic formulas. 
	The measure atomic formulas require some weight to be computed over all existing points. Because of this a top to bottom approach is chosen for this 
	algorithm, where all valid modal points are generated for the variables in the branch conjunction.
	Same as in the definition for the model without measure formulas a modal point is represented by a variable valuation function. 
	To prove that a model exists each subset of valid modal points is tested whether it satisfies the branch conjunction.

	\begin{defn}
	Let B be a branch conjunction. We say that the modal point p, represented with a variable valuation function $\curvedE$ is valid, if the following statements hold:
	\begin{itemize}
	...
	\end{itemize}
	Let W be the set of all valid points, without duplication. The number of W has an upper bound of $2^n$, where n is the number of different variables in the branch
	conjunction. To determine all valid relations between the valid points we need to figure out each variable valuation first.
	\end{defn}

	\begin{defn}
	Each valid point p is represented with a variable valuation function $\curvedE$ which means that for a given variable m we can 
	determine if $p \in \vE(m)$ with $\vBool_{\curvedE}(m)$. Doing this for each variable and for each valid modal point defines the valid model valuation $\vE$.
	This procedure is done by looping over each valid modal point p and extending $\vE$ for each variable set in the variable valuation of the modal point p.
	\begin{align*}
		\vE(m) = \{ p \; | \; \vBool_{\curvedE}(m) \; \textit{is true} \}
	\end{align*}
	\end{defn}

	\begin{defn}
	Let B be a branch conjunction. Let W be the set of all valid points and $\vE$ be the valid valuation function constructed from W.
	Then a valid model relation R is defined by creating a relation between each modal point except for the modal points which are not in contact.
	\begin{align*}
		\neg C(e, f) \in B: (\forall x \in \vE(e) \textit{ and } \forall y \in \vE(f) \: \neg xRy)
	\end{align*}
	\end{defn}

	\begin{defn}
	Let B be a branch conjunction. Let $\mathcal{M} = (W, R, \vE)$ be a model defined by all valid modal points and the corresponding 
	valuation and relation functions over the branch conjunction B.
	$\mathcal{M}$ is a possible model which satisfies the branch, which means that it satisfies the formula. 
	In case the system of inequalities over $\mathcal{M}$ and B has a solution, then this model satisfies the initial formula.
	However in the opposite scenario, where the system of inequalities does not have a solution it does 
	not mean that the branch conjunction B does not construct a model.
	\end{defn}

	It is possible that the satisfying model has less modal points. 
	This means that we should test the satisfiability of the branch conjunction against all models produced from the modal points for each subset of W.
	...
	\begin{lemma}[Impossible subset models] \label{impossible-subset-models}
		Let B be a branch conjunction. Let  $\mathcal{M} = (W, R, \vE)$ be a model,
		where W is a subset of all valid modal points over B, then the following statements hold:
	\begin{enumerate}
		\item $t \neq 0 \in B: \vE(t) = \emptyset \implies (\forall W' \subseteq W : W' \neq \emptyset) \; \vE'(t) = \emptyset$
		\item $C(a,b) \in B: \vE(a) = \emptyset \implies (\forall W' \subseteq W) \; \vE'(a) = \emptyset$
		\item $C(a,b) \in B: \vE(b) = \emptyset \implies (\forall W' \subseteq W) \; \vE'(b) = \emptyset$
	\end{enumerate}
	\end{lemma}

	\begin{proof}
		A proof for the first equation is given, the second and third equation follow analogously.
	\newline
		Let us take one formula $t \neq 0$ from B, such that $\vE(t) = \emptyset$. To proove that for all subsets of W, 
		the valuation remains the empty set we shall use a structural induction over t.
		Let W' be defined as $W \backslash \{ p \}$ for an arbitrary modal point p. This way we will see that if we remove an arbitrary modal point from
		W the lemma holds and for arbitrary modal points removed from W, namely for every subset of W will follow analogously.
		By structural induction we have: 
		\begin{itemize}
			\item $t \equiv 1$, this is not possible, since $\vE(t) = \emptyset$
			\item $t \equiv 0$, then $\vE'(t) = \emptyset$ as well
			\item $t \equiv a$, where a is a variable and $\vE(a) = \emptyset$ \\ 
				from $\vE(a) = \emptyset$, it follows that $\neg \exists q \in W: q \in \vE(a)$ \\
				$\Rightarrow p \notin \vE(a)$ \\
				$\Rightarrow p \notin \vE'(a)$ \\
				$\Rightarrow \vE(a) = \vE'(a) = \emptyset$
			\item $t \equiv \neg u$, where u is a term and $\vE(\neg u) = \emptyset$ \\
				from $\vE(\neg u) = W \backslash \vE(u) = \emptyset$ it follows that $\vE(u) = W$ \\
				$\Rightarrow \neg \exists q \in W: q \in \vE(\neg u )$ \\
				$\Rightarrow \forall q \in W: q \in \vE(u)$, from $W' = W \backslash \{p\}$, it follows that \\
				$ \neg \exists q \in W': q \in \vE'(\neg u )$ \\
				$ \forall q \in W': q \in \vE'(u)$ \\
				$\Rightarrow \vE'(u) = W'$
				$\Rightarrow \vE'(\neg u) = \emptyset$
			\item $t \equiv u \sqcap w$, where u and w are terms and $\vE(u \sqcap w) = \emptyset$ \\
				$\Rightarrow \neg \exists q \in W: q \in \vE(u) \textit{ and } q \in \vE(w)$ \\
				$\Rightarrow \forall q \in W: q \notin \vE(u) \textit { or } q \notin \vE(w)$, from $W' = W \backslash \{p\}$, it follows that \\
				$\Rightarrow \forall q \in W': q \notin \vE'(u) \textit { or } q \notin \vE'(w)$
				$\Rightarrow \vE'(u \sqcap w) = \emptyset$
			\item $t \equiv u \sqcup w$, where u and w are terms and $\vE(u \sqcup w) = \emptyset$ \\
				$\Rightarrow \neg \exists q \in W: q \in \vE(u) \textit{ or } q \in \vE(w)$ \\
				$\Rightarrow \forall q \in W: q \notin \vE(u) \textit { and } q \notin \vE(w)$, from $W' = W \backslash \{p\}$, it follows that \\
				$\Rightarrow \forall q \in W': q \notin \vE'(u) \textit { and } q \notin \vE'(w)$
				$\Rightarrow \vE'(u \sqcup w) = \emptyset$
		\end{itemize}
	\end{proof}
	\begin{lemma}[Valuation deduction] \label{valuation-deduction}
		Let B be a branch conjunction. Let  $\mathcal{M} = (W, R, \vE)$ be a model, where W is a set of all valid modal points over B.
		Let $\{ q_0, ... q_n \}$ be valid modal points and let W' be defined as $W' = W \backslash \{ q_0, ... q_n \}$, then $\vE'$ can be calculated as:
		\begin{itemize}
			\item $t \neq 0 \in B: \vE'(t) =  \vE(t) \backslash \{ q_0, ... q_n \}$
			\item $C(a,b) \in B: \vE'(a) =  \vE(a) \backslash \{ q_0, ... q_n \} \textit{ and } \vE'(b) =  \vE(b) \backslash \{ q_0, ... q_n \}$
			\item $a \leq_\mu b \in B: \vE'(a) =  \vE(a) \backslash \{ q_0, ... q_n \} \textit{ and } \vE'(b) =  \vE(b) \backslash \{ q_0, ... q_n \}$
			\item $a <_\mu b \in B: \vE'(a) =  \vE(a) \backslash \{ q_0, ... q_n \} \textit{ and } \vE'(b) =  \vE(b) \backslash \{ q_0, ... q_n \}$
		\end{itemize}
	\end{lemma}
	
	This lemma implies that the valuation for each subset of W can be deduced from $\vE$ with less computations.

	\subsection{Satisfiability Algorithm}
	The satisfiability of a measure formula is done with an iteration over the possible subsets of valid modal points in a search for the one which constructs a model satisfing the formula.This algorithm is defined with the following steps.

	\paragraph{1. Branch Conjunction} Using the tableau method iterate over each branch conjunctions B.

	\paragraph{2. All Valid  Modal Points} Generate all valid modal points W over B. Following the definitions above the set W of valid modal points implies the model $\mathcal{M} = (W, R, \vE)$.

	\paragraph{3. Precomputed valuations} Compute ahead the valuations of the $t \neq 0, C(a,b), a \leq_\mu b, a <_\mu b$. These computed values will be used to quickly compute the valuations for a submodel with subset of valid modal points. This is done according to the valuation deduction lemma \ref{valuation-deduction}.

	\paragraph{4. Loop Over the Possible Subsets}  For each subset $W' \subseteq W$ test if it satisfies the $t \neq 0$ and C(a,b) atomic formulas from the branch conjunction B:
	\begin{itemize}
	\item In case it satisfies the $t \neq 0$ and C(a,b) verify whether the system of inequality has a solution. If the system has a solution then a valid model for the formula is constructed, otherwise continue with the next subset $W'$.
	\item If $W'  \subseteq W$ does not satisfy the $t \neq 0, C(a,b)$ then by lemma \ref{impossible-subset-models} all $W'' \subseteq W'$ are eliminated.
	\end{itemize}
	\paragraph{5. Final model} If W' is found such that it satisfies $t \neq 0, C(a,b)$ and the system of inequality then a valid model has been constructed for the input formula.
