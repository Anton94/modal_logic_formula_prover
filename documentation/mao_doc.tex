...
\subsection{Measure formula satisfiability}
	...
	Let W be a set of valid points, then for each valid point $\curvedE \in W$ we can determine if it is present in the model valuation of an arbitrary term t. Namely we can define the model valuation $\vE$ as follows:
	\begin{align*}
		\vE^W(t) = \{ p \; | \; \vBool_{\curvedE}(t) \; \textit{is true} \}
	\end{align*}
	\end{defn}
