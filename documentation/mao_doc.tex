...
\subsection{Measure formula satisfiability}
	...
	\begin{defn}
		Let B be a branch conjunction. We say that $\langle x, y \rangle$ is a valid relation if the following holds:
		\begin{align*}
	...
	\begin{lemma}[Impossible subset models] \label{impossible-subset-models}
		...
		Let $\mathcal{M'} = (W', R', \vE')$ be a model, where $W' \subseteq W$, then the following statements hold:
	\begin{enumerate}
	...
	\begin{lemma}[Valuation deduction] \label{valuation-deduction}
		Let  $\mathcal{M} = (W, R, \vE)$ be a model, where W is a set of valid modal points.
		Let $W' \subseteq W$ , then for the terms in the non-equal to zero, contacts, measure less or equal and measure less formulas the $\vE'$ can be calculated as follows:
		\begin{itemize}
			\item $t \neq 0: \vE'(t) =  \vE(t) \cap W'$
			\item $C(a,b): \vE'(a) =  \vE(a) \cap W' \textit{ and } \vE'(b) =  \vE(b) \cap W'$
			\item $a \leq_\mu b: \vE'(a) =  \vE(a) \cap W' \textit{ and } \vE'(b) =  \vE(b) \cap W'$
			\item $a <_\mu b: \vE'(a) =  \vE(a) \cap W' \textit{ and } \vE'(b) =  \vE(b) \cap W'$
		\end{itemize}
	\end{lemma}
...
%		\item At this point we have constructed the model over all valid modal points $\mathcal{M} = (W, R, \vE)$
		\item We are going to look at subsets of W which might produce a valid model, while considering, that the subsets with more elements have priority.

			For each subset $W' \subseteq W$, implying the model $\mathcal{M'}$ test if it satisfies the non-equal to zero terms and contacts from B:
		\begin{enumerate}
		\item In case the non-equal to zero terms and contacts from B are satisfied by $\mathcal{M'}$,
			...
			If the system has a solution then a valid model for the formula is constructed, otherwise continue with the next subset from W.
