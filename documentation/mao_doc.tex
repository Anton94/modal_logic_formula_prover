...
The satisfiability verification for a given formula is done by constructing a model. The formula is satisfiable only if such a model exists. The model construction is done in several steps. Foremost, the tableau method is used to simplify the problem and to detect contradictions earlier in the process, even before the model construction. If all branches of the tableau method are closed, then a model in which the given formula is satisfiable does not exist. If there are open branches in the tableau result, then semantical reasoning shall be done in order to construct a model, if such a model exists. Let us define few definitions before we define the process of formula satisfiability.
	...
	\begin{defn}
		Let <$\curvedE$, a> be a representation of a modal point p. Then the following holds:
		\begin{align*}
	...
	With definitions (3.8, 3.9) we are able to construct a model where the contacts and non zero terms from the branch conjunction are satisfied.
	We will denote this model as a \textbf{partial model}. The points in the partial model are represented with a tuple <$\curvedE$, a>.
           Furthermore, it must be verified that the modeled <$\curvedE$, a> for each model point satisfy the equal to zero terms and the non contacts from the branch conjunction.

	\begin{defn}
		Let M be a partial model. Then we say that the partial model M satisfies an equal to zero term $a = 0$, if the following holds:
		\begin{align*}
			\forall \textit{<$\curvedE$, a>} \in M: \vBool_{\curvedE}(a) \ne \emptyset
		\end{align*}
	\end{defn}

	\begin{defn}
		Let M be a partial model. Then we say that the partial model M satisfies a non contact $\neg C(a, b)$, if the following holds:
		\begin{align*}
			\forall \textit{<$\curvedE$, a>} \in M: \vBool_{\curvedE}(a) = \emptyset \lor \vBool_{\curvedE}(b) = \emptyset
		\end{align*}
	\end{defn}		
