...
The satisfiability verification or sometimes called a satisfiability proof for a given formula is done by constructing a model. The formula is satisfiable only if such a model exists. The model construction is done in several steps. Foremost, the tableau method is used to simplify the problem and to detect contradictions earlier in the process, even before the model construction begins. If the produced tableau is closed as per definition \ref{closed-tableau}, then a model in which the given formula is satisfiable does not exist. If there are open branches in the tableau result, then semantical reasoning shall be done in order to construct a model, if such a model exists. Let us define few definitions before we dive into the procedure for the satisfiability proof of a formula.
	...
	\begin{tikzpicture}[scale = 1]
      ...
      \end{tikzpicture}
	\end{center} 
% TODO fix this it should be steps not definitions
	...
	The modal points in the model are represented with a tuple $\langle \curvedE, a \rangle$.
	For a model constructed with definitions \ref{def-non-zero-construction} and \ref{def-contacts-construction} to satisfy the formula we must verify that each of the modeled  $\langle \curvedE, a \rangle$ modal point satisfies the equal to zero terms and the reflexivity of the non contacts from the branch conjunction.

	\begin{defn}
	\label{def-equal-to-zero}
		Let M be a model constructed with definition \ref{def-non-zero-construction} and \ref{def-contacts-construction}. Then we say that the model M satisfies an equal to zero term $a = 0$, if the following holds:
		\begin{align*}
			\forall \langle \curvedE, a \rangle \in W: \vBool_{\curvedE}(a) = \textit{false}
		\end{align*}
	\end{defn}
