...
\subsection{Measure formula satisfiability}
	...
	Where a modal point is represented by a variable valuation function. To prove that a model exists each subset of valid modal points 
	is tested whether it satisfies the branch conjunction.
	...
	conjunction. To determine all valid relations between the valid points we need to figure out 

	\paragraph{Valid modal valuation}
	...
	determine if $p \in \vE(m)$ with $\vBool_{\curvedE}(m))$. Doing this for each variable and for each valid modal point defines the valid $\vE$.
	...
	\paragraph{Valid modal relation}
	...
	Then a valid modal relation R is defined by creating a relation between each modal point except for the modal points which are not in contact.
	\begin{itemize}
	...
	M is a possible model which satisfies the branch, which means that it satisfies the formula. 
	In case the system of inequalities over M and B has a solution, then this model satisfies the initial formula.
	...
	\begin{lemma}[Impossible subset models] \label{impossible-subset-models}
		...
		\begin{align*}
			C(a,b) \in B: \vE(a) = \emptyset \textit{ or } \vE(b) = \emptyset \implies (\forall W' \subseteq W) \vE'(a) = \emptyset \textit{ or } \vE'(b) = \emptyset
		\end{align*}
	\end{lemma}
	...
	\begin{lemma}[Valuation deduction] \label{valuation-deduction}
		...
		\begin{itemize}
			\item $t \neq 0 \in B: \vE'(t) =  \vE(t) \{ q_0, ... q_n \}$
			\item $C(a,b) \in B: \vE'(a) =  \vE(a) \{ q_0, ... q_n \} \textit{ and } \vE'(b) =  \vE(b) \{ q_0, ... q_n \}$
			\item $a \leq_\mu b \in B: \vE'(a) =  \vE(a) \{ q_0, ... q_n \} \textit{ and } \vE'(b) =  \vE(b) \{ q_0, ... q_n \}$
			\item $a <_\mu b \in B: \vE'(a) =  \vE(a) \{ q_0, ... q_n \} \textit{ and } \vE'(b) =  \vE(b) \{ q_0, ... q_n \}$
		\end{itemize}
	\end{lemma}
	...
	\begin{enumerate}
		...
		\item Precompute the valuation of the $t \neq 0, C(a,b), a \leq_\mu b, a <_\mu b$ formulas \ref{valuation-deduction}
		\item For each subset $W' \subseteq W$ test if it satisfies the $t \neq 0, C(a,b)$ and the system of inequality.
		\item If a subset $W'  \subseteq W$ does not satisfy the $t \neq 0, C(a,b)$ by lemma \ref{impossible-subset-models} then all $W'' \subseteq W'$ are eliminated.
		\item If a W' is found such that it satisfies $t \neq 0, C(a,b)$ and the system of inequality then a valid model has been constructed for the input formula.
	\end{enumerate}
