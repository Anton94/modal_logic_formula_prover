...
variable_id_to_points_t variable_evaluations_;
	...
	\subsection{Connectivity Model}
		...
		1) Let $\mathbb{M}$ denote the model of all possible unique model points, where one model point corresponds to 
			...
			1.1) Filter out the model points which interfere with the zero terms, which means that:
				 \begin{equation}
					if P_i evaluates some term G_l to constant true then P_i is filtered out.
				\end{equation}
		...
		2) Verification is done for the Not Equal to Zero atomic formulas in $\mathbb{M}$, namely:
			\begin{equation}
				If \exists k: v(D_k) \neq \empty, then there does not exist a connected model.
			\end{equation}
		...
		3) Let $\mathbb{C}$ be a bit matrix of size $2^N x 2^N$.
			...
			\begin{equation}
				Model points i and j are in contanct if \mathbb{C}[ i ][ j ] is set.
			\end{equation}
			Let $\forall i, j \in \mathbb{N} : \mathbb{C}[ i ][ j ] = 1$
			...
			3.1) Remove all connections which interfere with Non Contact atomic formulas.
				...
				\begin{equation}
					\forall a, b < 2^N, \forall j : 
						if P_a [E_j] = 1 \wedge P_b [F_j] = 1 \vee P_a[F_j] = 1 \wedge P_b[E_j] = 1,
						then \mathbb{C}[a][b] = 0 and \mathbb{C}[b][a] = 0.
				\end{equation} 
		...
		4) Check that all Contacts are still satisfied
			\begin{equation}
				\forall i, j \in \mathbb{N}: C(A_i, B_i) is satisfied
			\end{equation}
		...
		5) The previous step produces the Maximal unique satisfiable model, wihch is a graph $\mathbb{G}$. 
			...
			Upper bound for Y is $2^N$, so the upper bound and on the time complexity is: $O(2^N * 2^N * M)$,
			since $x * x * M + y * y * M <<< (x+y)*(x+y)*M$ ????????
