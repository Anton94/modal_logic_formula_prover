...
variable_id_to_points_t variable_evaluations_;
	...
	\subsection{Meassured Model}
		...
		The first step is to construct the points which will represent the possible meassured model.
		The following atomic formulas produce an existance of a point:
		\begin{itemize}
			\item C(x, y) produces two model points, \\
				where x and y are terms.
			\item $x != 0$ produces one model point, \\
				where x is a term.
			\item Each measured atomic formula produces one model point
		\end{itemize}
		...
		\newline
		For each Not Equal to Zero atomic formula one new modal point is created.
		\newline
		For each measured atomic formula one new point is added % TODO add detailed description why is this new one point
		\newline
		Each Point will be represented with $P(\tau , e)$, where $\tau$ is a term and e is the variable evaluation.
		The point's evaluation P[e] positively evaluates the point's term $P[\tau ]$, this means that in order for the 
		point to be valid it's term must be evaluated to true. This means that P belongs to $v(P[\tau ])$.
		\newline
		...
		\newline
		Each point must keep track of it's associated term in order to be able to generate next evaluation which evaluates the term to 'true'. % TODO what is next eval.
		The search for satisfiable model is done in steps, where in each steps a set of evaluation is tested for satisfiability,
		if this test fails then new set of point evaluations must be generated. 
		The mechanics behind the next evaluation generation will be described further.
		...
		\subsubsection*{Measured Model Algorithm}
			...
			2) For each $D_k \neq 0$ one point is created P: $P[D_k] = true$.
				If not possible - then there is no satisfiable model.
			\newline

			3) If the points are less than the measured atomics*2 (or zero points) then dummy points are 
				created which are associated to '1' constant term, 
				just to make real points and to be able to put at least 
				one variable in both sides of the inequalities. % TODO
			\newline
			\newline
			At this point there is at least one point in the model.
			\newline
			...
			6) For each $G_l \neq 0$ evaluate $v(G_l)$ and check if it's the empty set. O(L)
			\newline

			7) For each $~C(E_j, F_j)$ check if $v(E_j)$ and $v(F_j)$ contain a common point or if one of the sets contains a point which is in contact with a point in the other set.
			\newline
			\newline
			...
			For each <=m(a, b) calculate v(a) and v(b), create an inequality of the following type: 
			\begin{align*}
				\sum_i X_i <= \sum_j X_j, \text{ where }i \in v(a) \vee j \in v(b).
			\end{align*}
			For each ~<=m(a,b) calculate v(a) and v(b), create an inequality of the following type: 
			...
			\newline
			The next generations look as follows:
			\begin{align*}
			...
			\newline
    			When the condition that the point's evaluation should evaluate the term to true is takein into account, then all evaluation which do not evaluate the 
			...
			Complexity: O((I+J) * G), where G is the complexity for finding a next evaluation for a point, 
			namely finding an evaluation which evaluates the point's term to true, 
			which is at most $2^N$.
			\newline

			After a successfull generation v(X) is recalculated for each X(variable) so it's (I+J)*N, namely for each point's evaluation 
			go through each bitset to add that point to the set of points of v(X)(where X is the variable which bit is set) % TODO: better explanation
			\newline
