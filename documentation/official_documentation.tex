...
auto tableau::satisfiable_step() -> bool
{
	...
	\section{Formula Model}
		...
		\paragraph{Evaluation Function} The evaluation function is represented with a variable\_evaluations\_ variable which type is as defined in \ref{model-representation:evaluation-function}.
		...
		\paragraph{Contact Matrix} The contact matrix is represented with a contact\_relations\_variable which type is defined in \ref{sec:contact-matrix}.
...
virtual auto create(const formulas_t& contacts_T, 
    ...
    -> bool = 0;
		...
		When a final model $\mathcal{M}$ is constructed which models the inputed formula, the information which describes the model is stored in the following two variables.
		\subsubsection*{Contact Matrix member variable}
			\begin{lstlisting}
/*
