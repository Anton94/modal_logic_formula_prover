...
\begin{document}
	...
	\label{sec:term-implementation-representation}
		The term can be represented in memory inductively by analogy to the definition REF-DEF-TERM. %TODO:THEORY
...
class Term 
{
  ...
  ...
	...
	\paragraph{Term hash variable} 
		Conducting a test whether two terms are equal in lexical way is an important procedure for the sake of performant satisfiability algorithms.
		...
		compare the whole structure of the terms. With the use of precalcaulated hash value this complexity can be reduced to an amortized complexity of O(1).
		...
		\begin{itemize}
			...
			\item if $h_1 = h_2$, then the structures of both terms are compared.
		\end{itemize}
		\noindent
	...
	\subsection*{Term Partial-Evaluation}
		...
		\begin{itemize}
			\item If $\sigma$ is the negation operation, and $\tau$ is a term, 
				then the constructed term is $\overline{\tau}$
...
auto find_in_T(const formula* f) const -> bool
				\end{lstlisting}
                			Checks if the formula $\varphi$ exists in any positive collection depending on the type of $\varphi$.
...
auto find_in_F(const formula* f) const -> bool
                			\end{lstlisting}
                			Checks if the formula $\varphi$ exists in any negative collection depending on the type of $\varphi$.
				Namely if $\varphi$ is of type:
                			\begin{itemize}
