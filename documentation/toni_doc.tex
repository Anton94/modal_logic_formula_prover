...
\begin{document}
	...
	\subsection{Branch conjunction model building}
		The branch conjunction formula $\beta$ is satisfiable if $\beta$ has a model $\M$. We have to construct such a model $\M$ = ($\F, \upsilon$) = ((W, R), $\upsilon$). The contacts and non-zero terms in $\beta$ require an existence of points. Let $C(a_i, b_i)$ be a conjunct in $\beta$. $C(a_i, b_i)$ implies the existence of two points 
		
		
		TODO: zapiski sa tova. != 0 implies the existence of one point. So, we need to be able to construct points on demand belonging to arbitary term. The point representation should be a convinenient one to be able to do that thing. Fix the N sets and just extend them on demand.
		We will look at the points in W not as individual cells, but as in which sets they participate in.		
		Find an easy way of adding points to the sets. With N sets there are $2^n$ combinations between them. So, the new point could be one of the $2^n$ variations.
		
		
		Let n be the number of the unique boolean variables in $\beta$. Let $\V_n$ be the set of them. 
		
		
		
		
		In order to define $\upsilon$ we need to assing a set of modal points to each boolean variable $x \in \V_n$. 
		
		The modal connectives $\leq$ and C are over the valuations of the terms in them. 

	\subsubsection{Modal points}
		
		
		With $n$ boolean variables we can uniquely identify $2^n$ objects. It is sufficient to define a modal point representation which satisfies it. For $n$ variables it should be able to give $2^n$ unique points.
		\begin{defn}
			Let $\V_n$ be a denumeratable set of $n$ boolean variables, as follows:
			\begin{equation}
				\label{boolean-evaluation}
				\V_n = \{x_1, x_2, \ldots , x_n \}
			\end{equation}
			A \textbf{boolean evaluation $\E_n$} for $n$ variables is a sequence of 1s and 0s, as follows:
			\begin{equation}
				\E_n = \; < e_1, e_2, \ldots , e_n >, \; where \; e_1, \ldots, e_n \in \{0, 1 \}
			\end{equation}
		\end{defn}
		For $n$ boolean variables there are $2^n$ unique boolean evaluations. We will define the modal points as boolean evaluations, i.e. each modal point is an unique boolean evaluation.
		\begin{defn}
			Let $\V_n$ be a denumeratable set of $n$ boolean variables. The set of \textbf{all unique points $W_n$} over $n$ variables is defined as follows:
			\begin{equation}
		...
		\begin{defn}
			Let $\E_n$ be a boolean evaluation for $n$ variables. Then $(\E_n)^i$ is the i-th element in the sequence $\E_n$.
		\end{defn}

		\begin{defn}
			\label{valuation-modal-points}
			Let $\V_n$ be a denumeratable set of $n$ boolean variables, as in \ref{boolean-evaluation}. Let $W_n$ be the set of all unique points over n variables as in \ref{all-unique-points}.We will define a \textbf{valuation $\upsilon_n : \V_n \rightarrow \p(W_n)$} inductively as follows:
			\begin{itemize}
				...
				\item Boolean variables
					\begin{itemize}
						\item $\upsilon_n(x_i) = \{ \E_n \mid \E_n \in W_n \land (\E_n)^i = 1 \}$, for $i \in \{1, \ldots , n \}$
					\end{itemize}
				\item Boolean terms
					\begin{itemize}
						\item $\upsilon_n(a \sqcap b) = \upsilon_n(a) \cap \upsilon_n(b)$
						\item $\upsilon_n(a \sqcup b) = \upsilon_n(a) \cup \upsilon_n(b)$
						\item $\upsilon_n(a^*) = W_n \setminus \upsilon_n(a) = -\upsilon_n(a)$
					\end{itemize}
			\end{itemize}
		\end{defn}
	...
	\subsection{Model building}
		...
		Let $\V_n$ be the set of variables in $\phi$. Let $\F$ = (W, R) be a relational system with $W \subseteq W_n$ and $R \subseteq$ $W^2$. Let $\M$ = ($\F, \upsilon$) be a model, where $\upsilon : \V_n \rightarrow \p(W)$ is a valuation defined as in \ref{valuation-modal-points} over $W$. There are four types of atomic formulas: contacts $C$, $\neg C$, zero terms and non-zero terms. Only the contacts and non-zero terms requires point existence. We will create points only for them. The pseudocode bellow implements the main idea for building $\M$ as a model of $\phi$. Note that there are some more details and optimizations which will be shown in the implementation section.
		\LinesNumbered
		\DontPrintSemicolon
		\begin{algorithm}
			\SetKwFunction{CheckNegContact}{CheckNegContact}
			\SetKwFunction{CheckZeroTerms}{CheckZeroTerms}
			\SetKwProg{Fn}{Function}{:}{}

			\Fn{\CheckNegContact{$p$}}{
				\For{$k\gets1$ \KwTo $K$ \KwBy $1$}{
					\tcc{Checking $\neg C(e_k, f_k)$}
					\uIf{$(p \in \upsilon_n(e_k) \land p \in \upsilon_n(f_k)$}{
						\KwRet false
					}
				}
				\KwRet true
			...
			\Fn{\CheckNegContact{$p_1$, $p_2$}}{
				\For{$k\gets1$ \KwTo $K$ \KwBy $1$}{
					\tcc{Checking $\neg C(e_k, f_k)$}
					\uIf{$(p_1 \in \upsilon_n(e_k) \land p_1 \in \upsilon_n(f_k)) \lor
						(p_2 \in \upsilon_n(e_k) \land p_2 \in \upsilon_n(f_k)) \lor
						(p_1 \in \upsilon_n(e_k) \land p_2 \in \upsilon_n(f_k)) \lor
						(p_2 \in \upsilon_n(e_k) \land p_1 \in \upsilon_n(f_k))))
						$}{
						\KwRet false
					}
				}
			...
			\Fn{\CheckZeroTerms{$p$}}{
				\For{$l\gets1$ \KwTo $L$ \KwBy $1$}{
					\tcc{Checking $g_l = 0$}
					\uIf{$(x \in \upsilon_n(g_l)$}{
						\KwRet false
					}
				}
				\KwRet true
			...
			$W \gets \emptyset$
			\;
			\tcc{Create points for the contacts}
			\For{$i\gets1$ \KwTo $I$ \KwBy $1$}{\label{contacts}
				\tcc{Processing $C(a_i, b_i)$}

				\For{$x \in \upsilon_n(a_i)$}{
					\For{$y \in \upsilon_n(b_i)$}{
						\tcc{Verify that the pair <x, y> does not breaks any $\neg C$ and zero term}
						\uIf{$\CheckNegContact(x, y) \land \CheckZeroTerms(x) \land \CheckZeroTerms(y)$}{
							$W \gets W \cup \{x, y \}$
							\;
			...
			\tcc{Create points for the non-zero terms}
			\For{$j\gets1$ \KwTo $J$ \KwBy $1$}{\label{non-zero-terms}
				\tcc{Processing $d_j \neq 0$}

				\For{$x \in \upsilon_n(d_j)$}{
					\tcc{Verify that x does not breaks any $\neg C$ and zero term}
					\uIf{$\CheckNegContact(x) \land \CheckZeroTerms(x)$}{
						$W \gets W \cup \{ x \}$
						\;
						$R \gets R \cup \{ <x,x> \}$
						\;
	...
	\paragraph{} Let us think of W and R as an undirected graph $G = (W, R)$. $W$ is the set of vertexes and R the set of edges. The connectivity theorem implies that the graph $G$ should be connected, i.e. for each vertex $x \in W$ there should be a path to each vertex in $W$.
	\begin{equation}
	...
	\paragraph{} Let $\F$ = (W, R) be a relational system with W $\neq$ $\emptyset$, R $\subseteq$ $W^2$. Let $\M$ = ($\F, \upsilon$) be a model, where $\upsilon$ is an valuation. In order $\M$ to be a \textit{connected model} the graph $G=(W,R)$ should be connected.
	...
	\subsection{Connected model building}
		...
		Let $\V_n$ be the set of variables in $\phi$. Let $\F$ = (W, R) be a relational system with $W \subseteq W_n$ and $R \subseteq$ $W^2$. Let $\M$ = ($\F, \upsilon$) be a model, where $\upsilon : \V_n \rightarrow \p(W)$ is a valuation defined as in \ref{valuation-modal-points} over $W$. In order $\M$ to be a connected model for $\phi$, $W$ should be non-empty and the graph $G=(W, R)$ should be connected (from \ref{connectivity-theorem-graph}). The main idea for building a connected model is as follows:
		\begin{itemize}
			\item Create the set $W$ from all points in $W_n$ which does not interfere with the $\neg C$ and zero terms in $\phi$:
			\begin{equation}
				W = \{ p \mid p \in W_n \land (\forall_l)(p \notin \upsilon_n(g_l)) \land (\forall_k)(p \notin \upsilon_n(e_k) \lor p \notin \upsilon_n(f_k))\}
			\end{equation}

			\item Validate that all contact terms are non-zero:
			\begin{equation}
				(\forall_i)(\exists_{p_1 \in W})(\exists_{p_2 \in W})(p_1 \in \upsilon(a_i) \land p_2 \in \upsilon(b_i))
			\end{equation}
			...
			\item If Comp has a graph G' which defines a model of $\phi$ $\M' = (W', R', \upsilon')$, then $\M'$ is a connected model. $\upsilon' : \V_N \rightarrow \p(W')$ is defained as $\upsilon$ but over $W'$. Note that we only have to check the non-zero terms and the contact atomic formulas because $\M'$ does not introduce new points or relations:
			\begin{equation}
			\begin{split}
				(\exists_{(W', R') \in Comp})((\forall_j)(\exists_{p \in W'})(p \in \upsilon(d_j))) \land (\forall_i)((\exists_{x \in \upsilon'(a_i)})(\exists_{y \in \upsilon'(b_i)})(xR'y \land yR'x))
			\end{split}
			\end{equation}
		\end{itemize}
