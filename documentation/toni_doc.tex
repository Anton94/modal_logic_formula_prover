...
\\
	...
	\end{step}
		...
		\begin{defn}
			Let x, y $\in W^v$ are valid modal points. Then $\pairXY$ is a $\mathbf{valid \; connected \; pair}$ of $\beta$ modal points if it preserves the satisfaction of non-contacts in $\beta$.
			\begin{equation}
			\neg C(e, f) \in \beta \rightarrow
		...
		\begin{lemma}
			Let t $\in \Tb$ be an arbitrary term. By the definition of $\eta$ and $\upsilon_n$ ifollows:
			\begin{equation}
	...
	The algorithm described bellow creates modal points and connections only for the non-zero and contact terms in $\beta$.
		...
		\begin{algorithm}
			...
			\For{$C(a, b) \in \beta$}{\label{contacts}
				\For{$x,\; y \in W^v$}{
					\uIf{$\eta(a, x) = 1 \land \eta(b, x) = 1 \land IsValidCon(x,y)$}{
						$W \gets W \cup \{x, y \}$
						\;
