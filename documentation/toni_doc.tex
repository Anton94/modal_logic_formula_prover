...
\end{lstlisting}
	...
	The formula is a binary tree. A \textbf{subformula} is a subtree in the formula's tree. The process of building a satisfiable model is computational heavy. A part of it is lexical comparing and traversing subformulas. The AST is convenient to modify the formula. These modifications are a preprocessing step. After them the formula will not be modifed any more. Then a few optimizations could be done.
	...
	\paragraph{Hasing subformulas}
	Conducting a test whether two subformulas are equal in a lexical way is an important procedure for the sake of performance. Such equality checks are required in various situations, such as:
	\begin{itemize}
		\item testing if two subformulas are equal
		\item checking if a subformula exists in a set of subformulas
	\end{itemize}
	Have in mind that the naive solution to do an equality check has complexity of O(n), where n is the size of the formula. By comparing the whole subformula. To reduce this complexity a precalcaulated hash value shall be used.
...
\end{lstlisting}
	...
	TODO: review the last section above!!\textbf{•}
