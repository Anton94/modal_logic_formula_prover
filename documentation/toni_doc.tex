...
\begin{document}
	...
	\defn A model $\M$ is a \textit{\textbf{model of a formula}} $\phi$ if $\phi$ is \textit{true} in $\M$.

	\defn If $\phi$ has a model $\M$, then $\phi$ is \textbf{satisfiable}.

	\defn $\M$ is a \textbf{model of a set of formulas} $A$ if $\M$ is a model of all formulas from A.

	\defn A formula $\phi$ \textbf{is \textit{true}} (or \textbf{valid}) in a frame $\F$ ($\F \models \phi$), if $\M \models \phi$ for all models $\M$ based on $\F$, i.e. for all valuations $\upsilon$ we have $\F$,$\upsilon \models \phi$.\
	...
	\theorem (Connectivity) Let b is a term. Then \par $b \neq 0 \land b \neq 1 \implies bCb*$

	\paragraph{} Let $\F$ = (W, R) be a relational system with W $\neq$ $\varnothing$ and R $\subseteq$ $W^2$. Let us recall the deficition of C: \par $aCb \iff (\exists x \in \upsilon(a)) (\exists y \in \upsilon(b))(xRy)$, where a and b are terms
	...
	\par$(\forall x \in W)(\forall y \in W)(\exists \pi(x, y))$ ($\pi$ is a path in $G$)
