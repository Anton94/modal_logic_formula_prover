...
\begin{document}
	...
	\subsection{Branch conjunction model building}
		...
		\begin{lemma}
			Let t $\in \Tn$ be an arbitrary term. Let W $\subseteq$ $W_n$. Let $\eta$ be a boolean valuation. Let $\upsilon_n$ be a valuation as in \ref{valuation-modal-points} . Let $\E_n \in W_n$. Then $\mathbf{\E_n \; extends \; \upsilon_n \; and \; W}$ as follows:
			\begin{align*}
				\E_n \; extends \; \upsilon_n \; and \; W \iff \eta(t, \E_n) = 1 & \\
				\iff E_n \in \upsilon_n(t) & \\
				\iff \E_n \in \E_t
			\end{align*}
		\end{lemma}
		...
		\begin{defn}
			Let x, y $\in W^v$. Then <x, y> is a $\mathbf{valid \; connected \; pair \; of \; \beta}$ modal points if does not break the satisfaction of non contacts in $\beta$.
			\begin{align*}
			(\forall_{\neg C(e_k, f_k) \in \beta})\neg(
		...
		\end{defn}
			...
			Let $\M = (W^v, R^v), \upsilon_n)$. Then $\M$ is a model of $\beta$ if the non-zero terms and contacts are satisfied:
			\begin{align*}
	...
	\subsection{Model building}
		...
		\begin{algorithm}
			...
			\For{$i\gets1$ \KwTo $I$ \KwBy $1$}{\label{contacts-old}
			...
			\For{$j\gets1$ \KwTo $J$ \KwBy $1$}{\label{non-zero-terms-old}
				\tcc{Processing $d_j \neq 0$}
