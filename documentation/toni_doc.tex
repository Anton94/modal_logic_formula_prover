...
\begin{document}
	...
	\par
	Here is the tableau expansion beginning with the signed formula $\SF (X \land Y) \Rightarrow (\neg X \land \neg Y)$.
...
\\
	...
	\begin{lemma}
		Let $\M^v = (\F^v, \upsilon_n)$ be a model. Let $G^v=(W^v, R^v)$ be the graph of $\F^v$. Let $G=(W,R) \subseteq G^v$, then:
		\begin{itemize}
