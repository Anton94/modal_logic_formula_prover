...
\begin{document}
	...
	\begin{lemma}
		A closed tableau for $\SF X$ is a tableau proof of $X$, meaning that $X$ is a \textbf{tautology}.
	\end{lemma}

	\par
	Here is the final stage of a tableau expansion beginning with the signed formula $\SF (X \land Y) \Rightarrow (\neg X \land \neg Y)$.
	\\ \TableauExampleCenter 1. $\SF(X \land Y) \Rightarrow \neg(\neg X \lor \neg Y)$
	\\ \TableauExampleCenter 2. $\ST X \land Y$
	\\ \TableauExampleCenter 3. $\SF \neg(\neg X \lor \neg Y)$
	\\ \TableauExampleCenter 4. $\ST X$
	\\ \TableauExampleCenter 5. $\ST Y$
	\\ \TableauExampleCenter 6. $\ST \neg X \lor \neg Y$
	\\
    ...
    \end{tikzpicture}
	\\
	\TableauExampleFirstChildOffset 7. $\ST \neg X$
	\indent \indent\indent 8. $\ST \neg Y$
	\\
	\TableauExampleFirstChildOffset 9. $\SF X$
	\indent\indent\indent 10. $\SF Y$

	\par
	The numbers are for reference purposes. Items 2 and 3 are from 1 by $\SF \Rightarrow $. 4 and 5 are from 2 by $\ST \land$. 6 is from 3 by $\SF \neg$. 7 and 8 are from 6 by $\ST \lor$. 9 is from 7 by $\ST \neg$. 10 is from 8 by $\ST \neg$.

	The tableau displayed above is closed, so the formula $(X \land Y) \Rightarrow (\neg X \land \neg Y)$ is a tautology.
	...
	\\ \TableauExampleCenter 1. $\SF ((X \Rightarrow Y) \Rightarrow ((Y \Rightarrow X) \Rightarrow \neg Y))$
	\\ \TableauExampleCenter 2. $\ST X \Rightarrow Y$
	\\ \TableauExampleCenter 3. $\SF ((Y \Rightarrow X) \Rightarrow \neg Y)$
	\\ \TableauExampleCenter 4. $\ST Y \Rightarrow X$
	\\ \TableauExampleCenter 5. $\SF \neg Y$
	\\ \TableauExampleCenter 6. $\ST Y$
	\\
	\begin{tikzpicture}[scale = 0.8]
		\draw (-7,0) (5,1);
        \draw[thick] (-3,0) -- (-0.4,1.5) -- (2.6,0);
    \end{tikzpicture}
	\\
	\indent\indent\indent\indent\indent 7. $\SF X$
	\indent\indent\indent\indent\indent\indent\indent 8. $\ST Y$
	\\
	\begin{tikzpicture}[scale = 1]
		\draw (-3.2,0);
        \draw[thick] (1,0) -- (0,1) -- (-1,0);
        \draw[thick] (3.6,0) -- (4.6,1) -- (5.6,0); %The second argument is height and the center point, the first is offset to the left from the center point and the thirth is offset to the right of the center point
    \end{tikzpicture}
	\\
	\indent\indent\indent 9. $\SF Y$
	\indent\indent 10. $\ST X$
	\indent\indent\indent 11. $\SF Y$
	\indent\indent 12. $\ST X$
	\par
	Item 2 and 3 are from 1 by $\SF \Rightarrow$, as are 4 and 5 from 3. Item 6 is from 5 by $\SF \neg$. Items 7 and 8 are from 2 by $\ST \Rightarrow$, as are 9 and 10 from 4. Items 11 and 12 are also from 4 by $\ST \Rightarrow$. The leftmost branch is closed because of 6 and 9. The left-right branch is closed because of 7 and 10. The right-left branch is closed because of 8 and 11. But the rightmost branch is not closed. Notice that every non-atomic signed formula has had a rule applied to it on this branch and there is nothing left to do. (For classical propositional logic it is sufficient to apply a rule to a formula on a branch only once.) In fact the branch yields a counterexample, as follows. Let $\upsilon$ be a propositional valuation that maps X to \textit{true} and Y to \textit{true} in accordance to 8 and 12. Now, we work our way back up the branch. Since $\upsilon(Y) = true$, $\upsilon(\neg Y) = false$, item 5. From $\upsilon(X) = true$ follows that $\upsilon(Y \Rightarrow X) = true$, item 4. From $\upsilon(Y) = true$ follows that $\upsilon(X \Rightarrow Y) = true$, item 2. Since $\upsilon(Y \Rightarrow X) = true$ and $\upsilon(\neg Y) = false$ we have $\upsilon((Y \Rightarrow X) \Rightarrow \neg Y) = false$, item 3. Finally, $\upsilon((X \Rightarrow Y) \Rightarrow ((Y \Rightarrow X) \Rightarrow \neg Y)) = false$, item 1.
	\par
...
\\
	...
	\subsection{Branch conjunction model building}
		...
		\begin{defn}
			A finite set of $n$ boolean variables $\V_n$ is defined as follows:
			\begin{equation}
		...
		\begin{defn}
			Let $\T$ be the set of all boolean terms.
		\end{defn}
		
		\begin{defn}
			Let the function $Var : \T \rightarrow \p(\V)$ gives all unique variables used in a term.
		\end{defn}

		\begin{defn}
			Let $\V_n$ be a finite set of n boolean variables. Then $\Tn$ is the set of all boolean terms with variables of $\V_n$.
			\begin{equation}
				\Tn = \{t \mid t \in \T \; and \;\ Var(t) \subseteq \V_n \}
			\end{equation}
		\end{defn}

		Let n be the number of the unique boolean variables in $\beta$. Let $\V_n$ be the set of them. There are four types of atomic formulas in $\beta$, namely contacts, non-contacts, zero terms and non-zero terms. Only contacts and non-zero terms require existence of modal points. The valuation $\upsilon$ should assign a set of modal points to each boolean variable $x \in \V_n$. Such that the contacts and non-zero terms be satisfied.

		Let t $\in \Tn$ such that $Var(t) = \V$. Let p $\notin$ W be a new modal point. Let us try to extend $\upsilon$ such as $p \in$ $\upsilon(t)$. By the valuation definition  $\upsilon(t)$ is a composition of intersections, unions, and compliments of valuations $\upsilon(x_i)$ for $x_i \in \V$. Therefore, the modal point p should be added to zero or more variable valuations depending on the boolean operations in the term t. With n boolean variables there are $2^n$ ways of adding the point p. Note that it might not be possible to adjust the variable valuations such that $p \in \upsilon(t)$. For example, if t is $a \sqcap a^* \neq 0$.
...
-			Then $X$ is the union of all evaluations of boolean variables in $\V_n$.
			\begin{equation}
				\label{X-all-variable-valuations}
				X = \bigcup\limits_{x_i \in \V_n}\upsilon(x_i)
			\end{equation}
		\end{defn}
		...
		\begin{defn}
			Let $\V_n$ be a finite set of $n$ boolean variables. The set of \textbf{all unique variable evaluations $W_n$} over $n$ variables is defined as follows:
			\begin{equation}
		...
		\begin{defn}
			...
			Let $\V_n$ be a finite set of $n$ boolean variables. Let $W_n$ be the set of all unique points over n variables. Then the \textbf{valuation $\upsilon_n : \V_n \rightarrow \p(W_n)$} is defined by the following inductive definition:
			\begin{itemize}
				...
				\item Boolean variables
					\begin{itemize}
						\item $\upsilon_n(x_i) = \{ \E_n \mid \E_n \in W_n \; and \; (\E_n)^i = 1 \}$, for $x_i \in \V_n$
					\end{itemize}
				\item Boolean terms
					\begin{itemize}
		...
		Let $\beta$ be a branch conjunction as in \ref{branch-conjunction} . Let $\V_n$ be the finite set of boolean variables in $\beta$. Let $\upsilon_n$ be a valuation as in \ref{valuation-modal-points} . Let $W \subseteq W_n$. Let R $\subseteq$ $W^2$ be a reflexive and symmetric relation.
		...
		\begin{lemma}
			The valuations of the non-contact terms in $\beta$ do not have a common modal point. The proof follows from the reflexivity of R.
			\begin{equation}
				\neg C(e, f) \in \beta \rightarrow (\upsilon_n(e) \cap \upsilon_n(f)) = \emptyset
			\end{equation}
		\end{lemma}

		\begin{defn}
			Let $\E_n \in W_n$. $\E_n$ is a $\mathbf{valid \; modal \; point}$ of $\beta$ if it preserve the satisfaction of the zero terms and non-contacts in $\beta$:
			\begin{gather*}
				g = 0 \in \beta \rightarrow \E_n \notin \upsilon_n(g_l) \\ and \\
		...
		\begin{defn}
			Let x, y $\in W^v$. Then $\pairXY$ is a $\mathbf{valid \; connected \; pair}$ of $\beta$ modal points if it preserves the satisfaction of non-contacts in $\beta$.
			\begin{align*}
			\neg C(e, f) \in \beta \rightarrow
			(x \notin \upsilon_n(e) \; or \; y \notin \upsilon_n(f)) \; and \\
			(x \notin \upsilon_n(f) \; or \; y \notin \upsilon_n(e)) \; and \\
			(x \notin \upsilon_n(e) \; or \; y \notin \upsilon_n(e)) \; and  \\
			(x \notin \upsilon_n(f) \; or \; y \notin \upsilon_n(f))\;\;\;\;\;\;
			\end{align*}
		\end{defn}
		...
		\begin{lemma}\label{satisfied-contacts-non-zero-terms}
			...
			\begin{gather*}
				C(a, b) \in \beta \rightarrow (\exists \pairXY \in R^v)(x \in \upsilon_n(a) \land y \in \upsilon_n(b))
				 \\ and \\
				 d \neq 0 \in \beta \rightarrow (\exists \E_n \in W^v)(\E_n \in \upsilon_n(d))
			\end{gather*}
		\end{lemma}

		\begin{defn}
			\label{eta-valuation}
			Let $\eta: (\Tn \times W_n) \rightarrow \{0, 1 \}$ be a function which defines whether a modal point is in the valuation of a term. Let t $\in$ $\Tn$. Let $\E_n \in W_n$. The inductive definition of $\eta$ on the structure of the term t is as follows:
			\begin{itemize}
				\item $\eta(0, \E_n) = 0$
				\item $\eta(1, \E_n) = 1$
		...
		\begin{lemma}
			Let t $\in \Tn$ be an arbitrary term. By the definition of $\eta$ and $\upsilon_n$ ifollows:
			\begin{equation}
	...
	The following algorithm builds a more convenient model. Creates modal points and connections only for the non-zero and contact terms in $\beta$.
		...
		\begin{algorithm}
			...
			\For{$C(a, b) \in \beta$}{\label{contacts}
				\For{$\pairXY \in R^v$}{
					\uIf{$\eta(a, x) = 1 \land \eta(b, x) = 1$}{
						$W \gets W \cup \{x, y \}$
						\;
						$R \gets R \cup \{\pairXX, \pairYY, \pairXY, \pairYX \}$
						\;
						\Goto{contacts}
					}
				}
	...
	The relational system $\F = (W, R)$ defines an unidrected graph. $W$ is the set of vertices and $R$ is the set of edges.
	...
	\label{connected-model-building}
		...
		Let $\F^v = (W^v, R^v)$ be a relational system and $\M^v = (\F^v, \upsilon_n)$ be a model in this system. $\M^v$ contains all valid modal points and valid connections between them w.r.t zero terms and non-contacts in $\beta$. $\M^v$ is a model of $\beta$ if the contacts and non-zero terms in $\beta$ are satisfied \ref{satisfied-contacts-non-zero-terms} . If $\M^v$ is not a model of $\beta$, then $\beta$ does not have a model. There is no way of adding new modal points or relations to satisfy the contacts and non-zero terms. Therefore, there is no connected model eigther.

	\begin{lemma}
		Let $\M^v = (\F^v, \upsilon_n)$ be a model. Let $G^v=(W^v, R^v)$ be the graph of $\F^v$. Let $G=(W,R) \subseteq G^v$.
		\begin{itemize}
			\item G preserves the satisfaction of the contacts in $\beta$ if
			\begin{gather*}
				C(a,b) \in \beta \rightarrow (\exists x \in \upsilon_n(a) \cap W)(\exists y \in \upsilon_n(b) \cap W)(xRy)
			\end{gather*}

			\item G preserves the satisfaction of the non-contacts in $\beta$ if
			\begin{gather*}
				\neg C(e,f) \in \beta \rightarrow \neg ((\exists x \in \upsilon_n(e) \cap W)(\exists y \in \upsilon_n(f) \cap W)(xRy))
			\end{gather*}

			\item G preserves the satisfaction of the zero terms in $\beta$ if
			\begin{gather*}
				d = 0 \in \beta \rightarrow \upsilon(d) \cap W = \emptyset
			\end{gather*}

			\item G preserves the satisfaction of the non-zero terms in $\beta$ if
			\begin{gather*}
				g \neq 0 \in \beta \rightarrow \upsilon(g) \cap W \neq \emptyset
			\end{gather*}
		\end{itemize}
	\end{lemma}

	\begin{lemma}
		Let $\M^v = (\F^v, \upsilon_n)$ be a model. Let $G^v=(W^v, R^v)$ be the graph of $\F^v$. Let $G=(W,R) \subseteq G^v$. If G preserves the satisfaction of the contacts, non-contacts, zero terms and non-zero terms in $\beta$, then G defines a model $\M = (\F(W, R), \upsilon_n)$ of $\beta$.
	\end{lemma}
	...
	\end{defn}
		...
		Let $\M^v$ be a model of $\beta$. Let $G^v=(W^v, R^v)$ be the graph of $\F^v$. $G^v$ preserves the satisfaction of zero terms and non-contacts.
		All connected components of $G^v$ preserve the satisfaction of the zero terms and non-contacts in $\beta$. If there is a connected component $G'=(W', R')$ which satisfied the contacts and non-zero terms in $\beta$, then G' defines a connected model $\M'$ of $\beta$. Where $\F' = (W', R')$ and $\M' = (\F', \upsilon_n)$.

		Let n be the number of elements in $W^v$. There are $2^n$ subgraphs of $G^v$. It is far from sufficient to check each one of them if it's connected and whether it satisfies the contacts and non-zero terms of $\beta$.
	...
	\begin{defn}
		Let $G'=(W', R')$ and $G''=(W'', R'')$ are gaphs. The union of these graphs is the graph $G=(W,R)=(W' \cup W'', R' \cup R'')$.
	\end{defn}
	...
	\end{lemma}
		...
		Let $Comp^{G^v}$ is the set of the maximal connected components of $G^v$. Let n be the number of elements in $W^v$. The number elements in $Comp^{G^v}$ is at most n. The maximal connected components of $G^v$ does not have common vertices and edges. Therfore, the splitting of G to it's maximal connected components could be done in a very efficient way.

		Each graph of $Comp^{G^v}$ satisfies the zero terms and non-contacts of $\beta$. They do not introduce new modal points netiher a connections. If $Comp^{G^v}$ contains a graph that satisfied the contacts and non-zero terms of $\beta$, then this graph defines a connected model of $\beta$. Otherwise, $\beta$ does not have a connected model.

	\begin{lemma}(Connected component extension)

		Let $G=(W, R)$ be a graph. Let and $G'=(W', R') \subseteq G$ be a connected component of G.
	...
	\begin{theorem}
		...
		\begin{proof}
			Let $G^v$ does not have a maximal connected component which defines a model of $\beta$. Let $\beta$ has a connected model $\M = (\F(W, R), \upsilon_n)$. Let $G=(W, R)$ be the graph defined by $\F$. From the definition of the connected model follows that G is connected. $G$ contains only valid modal points and valid relations between them. $G^v$ contains all valid modal points and all vlaid connections between them. Therfore, $G \subseteq G^v$ and G is a connected component of $G^v$. By the connected component extension lemma G can be extended to a maximal connected component $G_m=(W_m, R_m)$ of $G^v$. The extension adds points from $W^v$ and relations from $R^v$ to $G_m$. These are only valid modal points and valid connections between them. Therefore, $G_m$ preserves the satisfaction of the non-contacts and zero terms in $\beta$. $G_m$ keeps the points and relations from $G$. Thus, $G_m$ preserves the satisfaction of the contacts and non-zero terms in $\beta$. Hence, $G_m$ defines a connected model $\M_m = (\F_m(W_m, R_m), \upsilon_n)$ of $\beta$. This leads to a contradiction with the assumption that $G^v$ does not have a maximal connected component which defines a model of $\beta$.
		\end{proof}
	\end{theorem}
