...
\begin{document}
	...
	\section{Implementation Introduction}
		...
		It should be easy and intuitive to write a formula. Only the keyboard keys should be used. The legend bellow describes the formula's syntaxis:
	...
	\end{itemize}
		...
		The formula is a sequence of characters. These characters does not give us any information for the formula's structure. Flex \& Bison is used to parse it into an AST (Abstract Syntax Tree). 

		Let $\phi = C(x1,x2) \;\& <=(x1, x3) => C(x1, x3)$. The following AST is for the formula $\phi$:
