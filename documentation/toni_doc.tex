...
\begin{document}
	...
	\\\indent\indent\indent\indent 1. $\SF(X \land Y) \Rightarrow \neg(\neg X \lor \neg Y)$
	\\\indent\indent\indent\indent 2. $\ST X \land Y$
	\\\indent\indent\indent\indent 3. $\SF \neg(\neg X \lor \neg Y)$
	\\\indent\indent\indent\indent 4. $\ST X$
	\\\indent\indent\indent\indent 5. $\ST Y$
	\\\indent\indent\indent\indent 6. $\ST \neg X \lor \neg Y$
	\\
	\begin{tikzpicture}[scale = 0.8]
		\draw (-4,0) (4,1);
        \draw[thick] (-1.5,0) -- (0,1) -- (1.5,0);
    \end{tikzpicture}
	\\\indent\indent\;\;  7. $\ST \neg X$  \indent \indent\indent 8. $\ST \neg Y$
	\\\indent\indent\;\; 9. $\SF X$ \indent\indent\indent 10. $\SF Y$

	\par
	In this we have added numbers for reference purposes. Items 2 and 3 are from 1 by $\SF \Rightarrow $. 4 and 5 are from 2 by $\ST \land$. 6 is from 3 by $\SF \neg$. 7 and 8 are from 6 by $\ST \lor$. 9 is from 7 by $\ST \neg$. 10 is from 8 by $\ST \neg$.
	\par
	Finally, the conditions for closing off a case (declaring a branch closed) are	simple. A \textbf{branch is closed} if it contains a contradiction, i.e. $\ST X$ and $\SF X$ for some formula $X$. A \textbf{branch is finished} if all formulas are applied, i.e. contains only signed variables(atomic formulas). A \textbf{branch is opened} if it is finished and does not contain any contradiction. If each branch is closed, then the \textbf{tableau is closed}. A closed tableau for $\SF X$ is a tableau proof of $X$, meaning that $X$ is a tautology. The tableau displayed above is closed, so the formula $(X \land Y) \Rightarrow (\neg X \land \neg Y)$ has a tableau proof.
	\par
	...
	\\\indent\indent\indent\indent\indent\indent 1. $\SF ((X \Rightarrow Y) \Rightarrow ((Y \Rightarrow X) \Rightarrow \neg Y))$
	\\\indent\indent\indent\indent\indent\indent 2. $\ST X \Rightarrow Y$
	\\\indent\indent\indent\indent\indent\indent 3. $\SF ((Y \Rightarrow X) \Rightarrow \neg Y)$
	\\\indent\indent\indent\indent\indent\indent 4. $\ST Y \Rightarrow X$
	\\\indent\indent\indent\indent\indent\indent 5. $\SF \neg Y$
	\\\indent\indent\indent\indent\indent\indent 6. $\ST Y$
	\\
	\begin{tikzpicture}[scale = 0.8]
		\draw (-5,0) (5,1);
        \draw[thick] (-2,0) -- (-0.4,1) -- (1.6,0);
    \end{tikzpicture}
	\\\indent\indent\indent 7. $\SF X$  \indent\indent\indent\indent\;\; 8. $\ST Y$
	\\
	\begin{tikzpicture}[scale = 0.8]
		\draw (-5,0) (5,1);
    ...
    \end{tikzpicture}
	\\\indent\;\; 9. $\SF Y$ \indent 10. $\ST X$  \indent 11. $\SF Y$ \indent 12. $\ST X$
	\par
...
\\
	...
	\subsection{Formula satisfiability}
		...
		\begin{defn}
			...
			Let $\phi$ be a formula. Let $\mathcal{T}$ be a tableau beginning with $\phi$. Let B be a set of all atomic signed formulas in a branch of $\mathcal{T}$, as follows:
			\begin{alignat}{2}
				B = \{\ST C(a_i, b_i) \mid i \in \{1, \ldots, I\} \} \cup
					\{\SF d_j = 0 \mid j \in \{1, \ldots, J\} \} \cup \\\nonumber
					\{\SF C(e_k, f_k) \mid k \in \{1, \ldots, K\} \} \cup
					\{\ST g_l = 0 \mid l \in \{1, \ldots, L\} \}
			\end{alignat}
		\end{defn}
...
-			Then $X$ is the union of all evaluations of boolean variables in $\V_n$.
	...
	\begin{defn}
		...
		\begin{equation}
			(\forall x \in W)(\forall y \in W)(x \neq y \implies (\exists \pi_G(x, y)) (\pi_G(x, y)\;is\;a\;path\;in\;G))
		\end{equation}
	\end{defn}
	...
	\begin{defn}
		Let $\F$ = (W, R) be a relational system. Let G = (W, R) be the undirected graph defined by $\F$. Let $\M = (\F, \upsilon_n)$ be a model of $\beta$. $\M$ is a \textbf{connected model} if $G = (W, R)$ is connected.
	\end{defn}
	...
	\label{connected-model-building}
		...
		Let $\F^v = (W^v, R^v)$ be a relational system and $\M^v = (\F^v, \upsilon_n)$ be a model in this system. $\M^v$ is a model of $\beta$ if the non-zero terms and contacts in $\beta$ are satisfied \ref{satisfied-contacts-non-zero-terms} .	If $\M^v$ is not a model of $\beta$, then $\beta$ does not have a model neither a connected model.
