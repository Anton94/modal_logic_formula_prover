...
\begin{document}
	...
	\subsection{Model building}
		...
		TODO: add info for the atomic formulas which require point creation

		Let $\F$ = (W, R) be a relational system with W $\neq$ $\emptyset$, R $\subseteq$ $W^2$ and $\M$ = ($\F, \upsilon$) be a model. The main idea for building $\M$ as a model of $\phi$ is as follows:
		\begin{itemize}
			\item For each atomic contact C(a,b) in $\phi$ we will create two points x and y such that $x \in \upsilon(a)$ and $y \in \upsilon(b)$. If they does not break any zero term or $\neg C$ atomic, then we will add them to $W$. If there are no such points, then there is no $\M$ model of $\phi$. The following formula verifies that x and y does not break any zero term or $\neg C$:
			\begin{alignat}{3}
				(\forall k \in \{1, \cdots , K \})(\neg((x \in \upsilon(e_k) \land x \in \upsilon(f_k)) \:\lor \\
				(y \in \upsilon(e_k) \land y \in \upsilon(f_k)) \:\lor \\
				(x \in \upsilon(e_k) \land y \in \upsilon(f_k)) \:\lor \\
				(y \in \upsilon(e_k) \land x \in \upsilon(f_k)))) \: \\ \land \\
				(\forall l \in \{1, \cdots , L \})(x \notin \upsilon(d_l) \land y \notin \upsilon(g_l))
			\end{alignat}
			\item For each non-zero term $d \neq 0$ in $\phi$ we will create a point x such that $x \in \upsilon(d)$. If the point x does not break any zero term or $\neg C$ atomic, then we will add it to $W$. If there is no such point, then there is no $\M$ model of $\phi$. The following formula verifies that x does not break any zero term or $\neg C$:
			\begin{alignat}{3}
				(\forall k \in \{1, \cdots , K \})(\neg((x \in \upsilon(e_k) \land x \in \upsilon(f_k)) \: \\ \land \\
				(\forall l \in \{1, \cdots , L \})(x \notin \upsilon(d_l))
			\end{alignat}
		\end{itemize}
