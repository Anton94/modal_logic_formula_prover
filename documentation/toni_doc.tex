...
\begin{document}
	...
	\subsection{Formula satsfiability}
		...
		\paragraph{}
		Let $\phi$ be a formula. Let us build a tableau beginning with $\phi$. Let the tableau has an opened branch $B$. The branch $B$ is a set of signed atomic formulas of the following type: $T C(a,b)$ $F C(e,f)$ $T a \leq b$ and $F a \leq b$, where a and b are terms. $B$ is an opened branch, so there are no contradicting formulas in it, i.e. $(\neg \exists X)(T X \in B \land F X \in B)$.
		...
		\begin{defn}
			...
			\begin{alignat}{2}
				X = \{T C(a_i, b_i) \mid i \in \{1, \ldots, I\} \} \cup
					\{F C(e_k, f_k) \mid k \in \{1, \ldots, K\} \} \cup \\\nonumber
					\{T g_l \mid l \in \{1, \ldots, L\} \} \cup
					\{F d_j \mid j \in \{1, \ldots, J\} \}
			\end{alignat}
		\end{defn}
	...
	\subsection{Modal points}
		...
		\begin{defn}
			...
			Let $\V_n$ be a denumeratable set of $n$ boolean variables, as in \ref{boolean-evaluation}. We will define a \textbf{valuation $\upsilon_n : \V_n \rightarrow \p(W_n)$} inductively as follows:
			\begin{itemize}
	...
	\subsection{Connected model building}
		...
		\begin{itemize}
			...
			\begin{equation}
			\begin{split}
				(\exists_{(W', R') \in Comp})((\forall_j)(\exists_{p \in W'})(p \in \upsilon(d_j))) \land ((\exists_{x \in \upsilon'(a_i)})(\exists_{y \in \upsilon'(b_i)})(xR'y \lor yR'x))
			\end{split}
			\end{equation}
		\end{itemize}
