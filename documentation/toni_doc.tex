...
\begin{document}
	...
	\subsection{Model building}
		...
		Let $\V_n$ be the set of variables in $\phi$. Let $\F$ = (W, R) be a relational system with $W \subseteq W_n$ and $R \subseteq$ $W_n^2$. Let $\M$ = ($\F, \upsilon$) be a model, where $\upsilon : \V_N \rightarrow \p(W)$ is a valuation defined as in \ref{valuation-modal-points} over $W$. There are four types of atomic formulas: contacts $C$, $\neg C$, zero terms and non-zero terms. Only the contacts and non-zero terms requires point existence. We will create points only for them. The pseudocode bellow implements the main idea for building $\M$ as a model of $\phi$. Note that there are some more details and optimizations which will be shown in the implementation section.
		\LinesNumbered
		\DontPrintSemicolon
		\begin{algorithm}
	...
	\subsection{Connected model building}
		...
		Let $\F$ = (W, R) be a relational system with W $\neq$ $\emptyset$, R $\subseteq$ $W^2$ and $\M$ = ($\F, \upsilon$) be a model. In order $\M$ to be a connected model for $\phi$ the graph $G=(W, R)$ should be connected (from \ref{connectivity-theorem-graph}). The main idea for building a connected model is as follows:
		\begin{itemize}
