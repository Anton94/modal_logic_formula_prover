...
\begin{document}
	...
	\subsection{Branch conjunction model building}
		...
		\begin{defn}
			...
			\begin{equation}
				\VX = \{\upsilon(x_i) \mid x_i \in \V_n \} = \{ \VXi \mid x_i \in \V_n \}
			\end{equation}
		\end{defn}
		...
		\begin{defn}
			...
			\begin{itemize}
				...
				\item Boolean variables
					\begin{itemize}
						\item $\upsilon_n(x_i) = \{ \E_n \mid \E_n \in W \land (\E_n)^i = 1 \}$, for $i \in \{1, \ldots , n \}$
					\end{itemize}
				\item Boolean terms
					\begin{itemize}
		...
		TODO: move to each definition which uses this things somehow..
		Let $\beta$ be a branch conjunction as in \ref{branch-conjunction} . Let W be a set of modal points. Let $\V_n$ be the set of boolean variables in $\beta$. Let $\upsilon_n$ be a valuation as in \ref{valuation-modal-points} . Let R $\subseteq$ $W^2$ be a relation.
		
		\begin{defn}
			The zero terms in $\beta$ are satisfied iff each zero term's evaluation is the empty set.
			\begin{equation}
				(\forall g_l \in \beta)(\upsilon_n(g_l) = \emptyset)
			\end{equation}
		\end{defn}
		...
		\begin{defn}
			Let $\V_n$ be a denumeratable set of boolean variables. Let $\Tn$ be the set of all boolean terms with variables of $\V_n$. Let $W_n$ be the set of all unique variable evaluations for n varialbes. By a \textbf{boolean valuation} of a boolean term in $\Tn$ we mean the function $\eta$. It assigns a boolean constant for each pair of term t $\in$ $\Tn$ and variable evaluation $\E_n$ $\in$ $W_n$.
			\begin{equation}
				\eta(t, \E_n) \in \{0, 1\}
			\end{equation}
			Let t $\in$ $\Tn$. Let $\E_n$ $\in$ $W_n$. The inductively definition of $\eta$ on the structure of the term t is as follows:
			\begin{itemize}
				\item $\eta(0, E_n) = 0$
				\item $\eta(1, E_n) = 1$
				\item $\eta(x_i, E_n) = (E_n)^i$		
				\item $\eta(a \sqcap b, E_n) = \eta(a, E_n) \land \upsilon(b, E_n)$
				\item $\eta(a \sqcup b, E_n) = \upsilon(a, E_n) \lor \upsilon(b, E_n)$
				\item $\eta(a^*, E_n) = \lnot \eta(a, E_n)$
			\end{itemize}
		\end{defn}
		
		TODO: define the set of all variables evaluations which evaluates term t to 1 

	\subsection{Model building}
		Let $\beta$ be a branch conjunction as in \ref{branch-conjunction} :
		\nonumber\BranchConjunction

		Let $\V_n$ be the set of variables in $\beta$. Let $\F$ = (W, R) be a relational system with $W \subseteq W_n$ and $R \subseteq$ $W^2$. Let $\M$ = ($\F, \upsilon$) be a model, where $\upsilon : \V_n \rightarrow \p(W)$ is a valuation defined as in \ref{valuation-modal-points} over $W$. There are four types of atomic formulas: contacts $C$, $\neg C$, zero terms and non-zero terms. Only the contacts and non-zero terms requires point existence. We will create points only for them. The pseudocode bellow implements the main idea for building $\M$ as a model of $\beta$. Note that there are some more details and optimizations which will be shown in the implementation section.
		\LinesNumbered
		\DontPrintSemicolon
		\begin{algorithm}
